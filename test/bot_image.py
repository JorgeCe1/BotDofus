...
def  coordenadas_vecinas(coord):
    x, y = coord
    return {(x, y), (x+1, y), (x-1, y), (x, y+1), (x, y-1)}

# Lee coordenadas de pantalla
def obtener_coordenadas_actuales(coord_actual, intentos=15, delay=0.3):
    if not buscando:
        return None

    posibles = coordenadas_vecinas(coord_actual)
    resultados = []
    ...
    for i in range(intentos):
        ...
        upscaled = cv2.resize(img_bgr, None, fx=3, fy=3, interpolation=cv2.INTER_CUBIC)
        pil_img = Image.fromarray(upscaled)
        ...
        if coords:
            print(f"🔍 OCR detectó: {coords}")
            if coords in posibles:
                print(f"✅ Coordenada válida encontrada: {coords}")
                return coords
            resultados.append(coords)
        else:
            print("❌ OCR no detectó coordenadas.")

        time.sleep(delay)

    # Si ninguna coincidencia exacta, mostrar estadísticas
    conteo = Counter(resultados)
    print(f"📊 Coordenadas más comunes: {conteo}")
    for coord, count in conteo.items():
        if coord in posibles:
            print(f"✅ Se acepta coordenada cercana más frecuente: {coord}")
            return coord

    print("⚠️ No se detectó una coordenada válida tras varios intentos.")
    return None
...
def al_fallar_3_veces():
    ...
    coord_actual = recorrer_zonas_definidas()
    print("⚠️ Se falló 3 veces. Verificando cambio visual en zona...")

    coord = obtener_coordenadas_actuales(coord_actual)
    print(coord)
